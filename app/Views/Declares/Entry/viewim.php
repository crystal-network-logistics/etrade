...
$domestic = get_overseas( $entry?$entry['domesticid']:0 );
...
<div class="panel-body">
    <form class="entity">
        ...
        <div class="row">
            <div class="col-md-3">
                <span>报关口岸: </span>
                <span class="text-primary"><?=\App\Libraries\Libcomp::get_dict('CITYEN',$entry["entryport"])?></span>
            </div>

            <div class="col-md-3">
                <span>启运国:</span>
                <span class="text-primary"><?=\App\Libraries\Libcomp::get_dict('COUNTRYEN',$entry["destionationcountry"])?></span>
            </div>
            <div class="col-md-3">
                <span>预计进口日期:</span>
            ...
            <div class="col-md-3">
                <span>贸易国(地区):</span>
                <span class="text-primary"><?=\App\Libraries\Libcomp::get_dict('COUNTRYEN',$entry["businesscountry"])?></span>
            </div>
        </div>
        <div class="row">
            <div class="col-md-3">
            ...
            <div class="col-md-3">
                <span>生产单位:</span>
                <span class="text-primary"><?=$entry["production"]?></span>
            </div>
        </div>
        ...
        <div class="row">
            <div class="col-md-6">
                ...
                <code class="text-primary">
                    <?php echo $overseas?$overseas['companyname']:'--'; ?>
                </code>
            </div>
            ...
            <div class="col-md-6">
                <label>境内收货人:</label>
                <code class="text-primary">
                    <?php echo $domestic?$domestic['companyname']:'--'; ?>
                </code>
            </div>
        </div>
        ...
        <?php if ( in_array( $entry['status'] , [3,4] ) && $has_project_back_update) : ?>
            <!--退回修改操作-->
        ...
        <?php if ($has_project_download && $entry['status'] > 2) :?>
            <?php $can_download = ((ckAuth() && $entry['status'] != 5)?true:(ckAuth( false )?true:false)) ?>
            ...
            <?php if( $can_download ) :?>
                ...
                ?>
                <!--下载报关资料-->
                <a class="btn btn-success hModal" data-yes="N" href="/declares/project/download_entry?id=<?=$project['ID']?>"
            ...
            <a class="btn btn-primary hModal"
               ...
               data-buttons='<?=json_encode($buttons)?>'>
                <?=($data['entry']['archives'] == 1)?'查看税单':'上传税单'?>
            </a>
            ...
            <?php if( in_array( $entry['status'] , [2,4] ) ): ?>
                ...
                <?php if ( $entry["status"] == 2 ):?>
                    <a class="btn btn-primary" 
                       href="/declares/project/upload_entry"
                       onclick="return comm.confirmCTL(this.href,'<?=($entry["status"] == 2)?'是否初审核通过?':'确认审核通过?'?>',(resp)=>{resp;setTimeout(()=>{window.location.reload()},3000)},{id:'<?=$project['ID']?>'})">
                        <?=($entry["status"] == 2)?'初审':'审核通过'?>
                    </a>
        ...
        <?php if($entry  && $entry['status'] != 5) {?>
            <a class="btn btn-primary" href="/declares/project/downbooking?id=<?=$project?$project['ID']:0?>" target="_blank">下载托书</a>
       <?php }?>
