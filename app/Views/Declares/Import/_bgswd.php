...
<body>
<?php $project = (object) $data['project']; $entry = (object) $data['entry']; $goods = $data['goods'];?>
...
<table style="width:100%;font-size:12px;vertical-align:top;border-collapse:collapse; table-layout: fixed">
    <tr>
        <td style="border: 1px solid black; width: 20%" class="cell_spacing">境内发货人<br /><?=$entry->production?></td>
        <td style="border: 1px solid black; width: 20%" class="cell_spacing">出境关别<br /> <?=\App\Libraries\LibComp::get_dict('CITYEN',$entry->entryport)?></td>
    ...
    <tr>
        <td style="border: 1px solid black; width: 20%word-break:break-all; word-wrap:break-all;" class="cell_spacing">境外贸易商<br />
            <?=($entry->businessman?$entry->businessman:(get_overseas($entry->businessid)['companyname']))?>
        </td>
    ...
    <tr>
        ...
        <td style="border: 1px solid black;" class="cell_spacing">贸易国(地区)<br/><?=\App\Libraries\LibComp::get_dict('COUNTRYEN',$entry->businesscountry)?></td>
        <td style="border: 1px solid black;" class="cell_spacing">运抵国(地区)<br/><?=\App\Libraries\LibComp::get_dict('COUNTRYEN',$entry->destionationcountry)?></td>
        <td style="border: 1px solid black;" class="cell_spacing">指运港<br/>
        </td>
        <td style="border: 1px solid black;" class="cell_spacing"></td>
    </tr>
</table>
...
<table class="tb">
    <tr>
        ...
        <td style="width: 13.33%" class="td">运费<br/><?=$entry->tranportexpense?></td>
        <td style="width: 13.33%" class="td">保费<br/><?=$entry->insurancefee?></td>
    ...
    <tr>
        <td colspan="8" style="padding:10px;border-left: 1px solid black;border-bottom: 1px solid black;border-right: 1px solid black;border-top: 1px solid black;">
            标记唛码及备注:
            <?=\App\Libraries\LibComp::get_dict('BGXS',$entry->entryform)?>
        </td>
    </tr>
</table>
...
<table class="tb" style="font-size:14px;">
    ...
    <?php foreach($goods as $key=>$row) {
        ...
        <tr>
            ...
            <td align="center" class="td" style="width: 10%"></td>
...
<table class="tb" style="margin-bottom:20px;">
    ...
    <tr>
        ...
        <td style="text-align: left;" class="cell_spacing">申报单位:</td>
    </tr>
</table>
</body>
</html>
