...
<style>
    ...
    .tbd1{border-right: 1px solid #000;border-bottom: 1px solid #000;padding:7px; font-size: 12px;width:<?php echo (100/(1 + (count($goods)>0?count($goods):5)))?>%}
</style>
<div style="position: relative">
    ...
    <div style="padding-left: 240px;">
        <p style="line-height:18px;font-size: 13px;">
           <?=($company ? $company->name : '--')?>&nbsp;<?=($company ? $company->ename : '--')?>
        </p>
        <p style="line-height:18px;font-size: 13px;">
            <?=($company ? $company->address : '--')?>&nbsp;<?=($company ? $company->enaddr : '--')?>
        </p>
        <p style="line-height:18px;font-size: 13px;">
            电话/TEL: <?=($company ? $company->contact : '--')?> 传真/FAX: --
        </p>
    </div>
</div>
...
<?php
    ...
    if($model && count($goods)) {
        foreach ($goods as $key => $item ) {
            ...
            $cots .= '<td class="tbd1">' . number_format($val->ProductAmount) . (is_numeric($val->productunit)?(\App\Libraries\LibComm::Unit($val->productunit)):$val->productunit).'</td>';
            $ckamt .= '<td class="tbd1">' . number_format($val->ProductAmount *  $val->ProductUnitPrice,2).' '. ($entry && ($entry->currency) ? $entry->currency : '').'</td>';
...
<table style="width: 100%;border-left: 1px solid #000;border-top: 1px solid #000;" cellspacing="0" cellpadding="0">
    ...
    <tr style=";height:80px">
        ...
        <td colspan="3" class="tbd">
            <?php if($model && $entry) {?>
            <?php $Bussniess = $entry?((object) get_overseas($entry->businessid) ):null;?>
