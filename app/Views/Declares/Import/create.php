        ...
        <div id="other" class="panel-collapse collapse in">
            <div class="panel-body">
                <?php if( $is_has_vii_create ): ?>
                    <a class="btn bg-indigo-300 hModal" href="/declares/vii/create?projectid=&customerid=<?=$CustomerID?>&isentrance=1" data-call="jump_page" data-text="保存">
                        <i class="icon icon-add"></i> 申请开票
                    </a>
                <?php endif;?>
                ...
                <?php if( $is_has_receipt_create ): ?>
                    ...
                    <a class="btn bg-indigo-300 hModal" href="/declares/receipt/create?id=&customerid=<?=$CustomerID?>&isentrance=1" data-buttons='<?=json_encode($buttons)?>' data-size="full"><i class="icon icon-add"></i> 新增收入</a>
                <?php endif;?>

                <?php if( $is_has_payment_create ): ?>
                    <a class="btn bg-indigo-300 hModal" href="/declares/payment/create?projectid=&customerid=<?=$CustomerID?>&isentrance=1" data-call="jump_page"><i class="icon icon-add"></i> 新增支付 </a>
                <?php endif;?>

                <?php if( $is_has_payment_create ): ?>
                    <a class="btn bg-indigo-300 hModal" href="/declares/paymentcl/create?projectid=&customerid=<?=$CustomerID?>&isentrance=1" data-call="jump_page"><i class="icon icon-add"></i> 新增成本支付 </a>
                <?php endif;?>
            </div>
        </div>
    </div>
