...
?>
<form id="frm_payment_rollcopy" class="form-horizontal" action="/declares/paymentcl/rollcopy">
    ...
    <div class="form-group">
        ...
        <div class="col-lg-10">
            <div class="input-group">
                <input class="form-control" name="amount" placeholder="可转出金额 <?=$payment_data['amount']?>" onkeypress="return comm.iNum()" required="required" min="0" max="<?=$payment_data['amount']?>">
                <a class="input-group-addon" onclick="$('#frm_payment_rollcopy input[name=amount]').val(<?=$payment_data['amount']?>)">全部转出</a>
            </div>
        </div>
    </div>
</form>
