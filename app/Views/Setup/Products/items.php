...
<div class="row" name="div" id="<?=$tag?>" style="border-radius: 2px">
    <div name="cld" style="background-color:rgb(243,245,249);margin-left:15px;margin-right:15px;margin-bottom:15px;overflow:auto;">
        <div class="customrow">
            ...
            <div class="productname">
                ...
                <span class="customtextbox"><label>品牌 : </label><input name="Goods[brand][]" value="<?=$item["brand"]?>" required="required" class="form-control inline input-xs input-120" style="padding: 3px;height: 28px"></span>
                <span class="customtextbox"><label>型号 : </label><input name="Goods[model][]" value="<?=$item["model"]?>"  required="required" class="form-control inline input-xs input-120" style="padding: 3px;height: 28px"></span>
                <span class="customtextbox"><label>英文名 : </label><input name="Goods[ProductEnglishName][]" value="<?=$item["englishname"]?>" required="required" class="form-control inline input-xs input-160" style="padding: 3px;height: 28px"></span>
            </div>
            <div class="linkarea"><a class="link hModal" data-yes="N" href="/setup/products/elements_detail?id=<?=$item['pid']?>" style="cursor:pointer">展开报关要素</a> | <a class="link" style="cursor:pointer" onclick="clone_elem('<?=$tag?>')">复制</a>|<a class="link" style="cursor:pointer" onclick="delGoods(<?=$tag?>)">删除</a></div>
        </div>
        <div class="customrow" style="padding-left:20px;padding-right:20px">
            <div class="div_select">
                <span class="customtextbox"><label>原产国(地区):</label><?=\App\Libraries\LibComp::select(($isim=='none' ? 'COUNTRYEN' :'COUNTRY'),['name'=>'Goods[region][]','style'=>'width:160px' ,'class'=>'select select-xs'],$item['region'],false)?></span>
        ...
        <div class="customrow" style="margin-bottom: 0px;">
            <table style="width: 100%;/*border:1px solid rgb(210,220,233);*/">
                ...
                <tbody>
                <tr>
                    <td style="padding:5px"><input type="hidden" name="Goods[id][]" value="<?=($item['gid']??'0')?>"><input name="Goods[ProductPackageAmount][]" value="<?=($item['ProductPackageAmount']??'')?>" onchange="table_input_calc()"  id="pack<?=$tag?>" size="10" onkeypress="return comm.iNum()" required="required"></td>
                    ...
                    <td style="padding:5px"><?=\App\Libraries\LibComp::select(($isim=='none' ? 'UNITEN' :'UNIT'),['name'=>'Goods[productunit][]',"style"=>'height:28px;padding:5px 3px;'],is_numeric($item["productunit"])?(\App\Libraries\LibComm::Unit($item["productunit"])):$item["productunit"])?></td>
                    ...
                    <td style="padding:5px;color:red" ><input name='Goods[ProductUnitTotalPrice][]' onchange="$(this).data('change','true');table_input_calc();" id="sum<?=$tag?>" size="10" data-change='false' value="<?php echo number_format(($item["ProductUnitTotalPrice"]?:($item["ProductAmount"]* $item["ProductUnitPrice"])),4)?>" onkeypress="return comm.iNum()"></td>
...
<script>
    ...
    $('#<?=$tag?> .div_select select:eq(0)').select2()
</script>
